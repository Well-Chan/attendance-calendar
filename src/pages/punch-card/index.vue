<template>
  <div class="punch-card">
    ...
    <date-picker :model-value="currentDay" @update:model-value="dateChanged">
      <template #label>{{datePickerLabel}}</template>
    </date-picker>
    <div class="tab-bars">
      <div
        class="bar"
...
import { ref, computed, reactive, onMounted, Ref, watch } from 'vue';
...
const datePickerLabel = computed(() => {
  if (currentMode.value === 'year') {
    return dayjs(currentDay.value).format('YYYY 年');
  }
  else if (currentMode.value === 'month') {
    return dayjs(currentDay.value).format('YYYY 年 M 月');
  }
})
...
watch(currentDay, (val) => {
  yearCalendarSwipeData.ref?.changeDate(val, currentMode.value === 'year');
  monthCalendarSwipeData.ref?.changeDate(val, currentMode.value === 'month');
})
