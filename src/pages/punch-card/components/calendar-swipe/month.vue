  ...
  <div class="month-calendar-swipe" ref="rootElRef">
    ...
    >
      <van-swipe-item
        ...
        :key="index"
      >
        <month-calendar class="month-calendar" ref="monthCalendarRefList" v-model="item.value" @click-date="clickDate">
...
const swipeIndexChange = (index: number): void => {
  ...
  if (newSwipeItem.value.getMonth() === oldSwipeItem.value.getMonth()) {
    // 若滑动前选中日期月份与滑动后月份相同，则日期采用滑动前选中日期，与clickDate点击相邻月份时触发滑动相呼应
