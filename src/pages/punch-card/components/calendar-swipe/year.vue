  ...
  <div class="year-calendar-swipe" ref="rootElRef">
    ...
    >
      <van-swipe-item
        ...
        :key="index"
      >
        <year-calendar class="year-calendar" ref="yearCalendarRefList" v-model="item.value" @click-month="clickMonth" @update:model-value="dateChanged(index, $event)">
...
const swipeItemList: Array<{ value: Date }> = reactive((() => {
  ...
  return [{ value: new Date(year - 1, 1, 1) }, { value: today }, { value: new Date(year + 1, 1, 1) }];
})())
...
const swipeIndexChange = (index: number): void => {
  ...
  const prevYearDate = new Date(newSwipeItem.value.getFullYear() - 1, 1, 1);
  const nextYearDate = new Date(newSwipeItem.value.getFullYear() + 1, 1, 1);
